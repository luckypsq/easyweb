...
<body>
	<%
		...
		for (int i = 0; i < files.getCount(); i++) {
			...
			diskPath = application.getRealPath("/back/upload");
		...
		for (String[] string : listS) {
			...
			/* if(string.length == 12){
				...
				book.setBimg(string[1]);
				book.setBname(string[2]);
				book.setBauthor(string[3]);
				book.setBcontent(string[4]+" "+string[5]);
				book.setBnum(Long.parseLong(string[6]));
				book.setBprice(Double.parseDouble(string[7]));
				book.setBuniversity(string[8]);
				book.setBucollege(string[9]);
				book.setBumajor(string[10]);
				book.setBclass(string[11]);
				...
				book.setBcontent(string[3]+" "+string[1]);
